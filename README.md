...
下面这部分是基于当前代码实际行为整理的，优先级高于历史描述。

### 1. 当前主链路

1. 拉流子进程持续把每一路视频解码成最新帧。
2. `camera.py` 里的 `StreamProcessor` 为每一路维护独立状态。
3. 检测结果由统一调度器异步返回，不是每帧都跑检测。
4. 当前配置下 `performance.detection_inference_interval = 3`，也就是约每 3 帧做一次检测。
5. 中间两帧不做位置预测，只复用最近一次真实检测确认下来的 track。
6. 如果某个目标在下一次检测里没再出现，对应 track 会进入 `missed > 0`，并且不会继续显示。
7. 渲染后的带框帧用于推流，也用于报警图片和报警视频生成。

### 2. 当前追踪策略

当前不是“光流预测补帧”，而是“低频检测 + 真实检测结果驱动的轻量 tracking”。

- 检测帧：用 YOLO 检测框更新 `SimpleTracker`
- 非检测帧：继续显示最近一次 `missed == 0` 的 track
- 一旦下一轮检测没有匹配到该目标，框立即不再显示
- 框上文字只显示：`类别 + 置信度`
...
- 报警图片：直接保存触发时那一张已经画好框的 `rendered_frame`
- 报警视频：总长 10 秒
- 视频内容强制整理为：`前 5 秒 + 报警图片对应帧 + 后 5 秒`
- 视频中间帧就是报警图片对应的触发帧

也就是说，报警图片和报警视频现在不是各自随便截取，而是绑定到同一个触发时刻。

### 4. 当前自动质检策略

报警素材生成后，`alert/alert_system.py` 会自动执行一次质检：

- 检查报警图片和触发帧的一致性
- 检查报警图片里对应框位置是否真的存在框色像素
- 检查报警视频中间帧和触发帧的一致性
...
所以现在新报警出来后，不需要人工先翻 `res`，日志会先给出一轮自检结果。

### 5. 当前关键文件

- 主流程：[camera.py](/d:/AI_code/ai_camera/camera.py)
- 轻量追踪：[tracking/simple_tracker.py](/d:/AI_code/ai_camera/tracking/simple_tracker.py)
- 报警与质检：[alert/alert_system.py](/d:/AI_code/ai_camera/alert/alert_system.py)
- 配置：[config/config.json](/d:/AI_code/ai_camera/config/config.json)

### 6. 流程图

```mermaid
flowchart TD
    A["视频流输入<br/>RTMP / RTSP"] --> B["FFmpeg 拉流子进程<br/>输出最新帧"]
    B --> C["StreamProcessor<br/>按路维护状态"]
    C --> D["统一推理调度器<br/>每 3 帧提交一次检测"]
    D --> E["YOLO 检测结果返回<br/>按 stream_key + frame_id 回写"]
    E --> F["SimpleTracker 更新轨迹<br/>只保留最近一次真实检测确认结果"]
    F --> G["渲染带框帧<br/>类别 + 置信度"]
    G --> H["推流线程<br/>输出 AI 视频流"]
    G --> I["报警处理"]
    I --> J["保存报警图片<br/>直接使用触发帧"]
    I --> K["生成 10 秒报警视频<br/>前 5 秒 + 触发帧 + 后 5 秒"]
    J --> L["自动质检<br/>图片一致性 + 验框"]
    K --> M["自动质检<br/>视频中间帧一致性"]
    L --> N["日志输出"]
    M --> N["日志输出"]
    N --> O["MinIO 上传 / 平台上报"]
```
